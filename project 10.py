...
corr_price = numerical_train.corr()['SalePrice'].sort_values(ascending=False)
...
# =====================================================================
train_num = train_set.select_dtypes(include=np.number)  # just a safe check...
train_cat = train_set.select_dtypes(exclude=np.number)
...
num_from_cat_vars = cat_to_num + bsmtTypes + ["BsmtExposure",
...
drop_vars = percent_missing_test.index[percent_missing_test > 10].tolist()
test_set.drop(drop_vars, axis=1, inplace=True)
...
test_cat = test_cat.drop(num_from_cat_vars, axis=1)
...
print(len(train_set_allnum.columns) == len(test_set_allnum.columns))